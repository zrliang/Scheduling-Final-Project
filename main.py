...
#     print(total_chromosomes[k].gene)
#     # job action
#     for j in range(len(jobs)):
#         jobs[j].set_gene(total_chromosomes[k].get_gene(j)) 
...
for x in range(num_iteration):
    ...
    select_index= Tournament_selection_get_select_index(sorted_total_chromosomes,rank_selection_num,population_size,elite_selection_size)
    #next Gen(elite & rank)
    chromosomes=[]
...
#print(chromosomes[0].gene)


# job set_probability
for j in range(len(jobs)):
    jobs[j].set_gene(chromosomes[0].get_gene(j)) 
